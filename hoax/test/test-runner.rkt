...
(define (test-runner run)
  ...
  ;; Literal string intern examples
  (check-equal? (run '(eq? "foo" "foo")) #t)
  (check-equal? (run '(eq? (car (cons "foo" "bar")) "foo")) #t)
  (check-equal? (run '(let ((x "foo"))
                        (eq? x "foo")))
                #t)
  (check-equal? (run '(let ((x "fff"))
                        (eq? x (make-string 3 #\f))))
                #f)
  (check-equal? (run '(let ((x (cons "foo" "foo")))
                        (eq? (car x) (cdr x))))
                #t)
